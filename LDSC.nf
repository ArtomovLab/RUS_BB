...
munge_params = []
phenotypes_json.eachWithIndex{ val, num -> munge_params.add( [num, val.phenocode.replaceAll('\\.', '__'), val.assoc_files[0], val.num_cases, val.num_controls,val.num_samples] ) }
...
process munge {
   ...
   set val(num), val(phenocode), val(filename), val(n_cases), val(n_controls), val(num_samples) from munge_params
   var k = "logistic"
   ...
   ${munge_exec} --chunksize 100000 --sumstats ${filename} --merge-alleles ${params.LDSC_snplist} --N ${num_samples} --p ${params.columns.pvalue} --signed-sumstats ${params.columns.effect},${params.no_effect} --snp ${params.columns.snp} --a1 ${params.columns.effect_allele} --a2 ${params.columns.other_allele} --out ${phenocode}
   fi
   """
